<template>
    <div>
        <el-card :body-style="{ padding: '15px' }">
            ...
            <el-table :data="block" border>
                <el-table-column label="名称" prop="title">
                </el-table-column>
                <el-table-column label="排序" prop="sortId">
                </el-table-column>
                <el-table-column label="数目">
                    <template scope="scope">
                        <el-tag type="success" v-for="item in scope.row.books||[]" >{{item}}</el-tag>
                    </template>
                </el-table-column>
        ...
        <el-dialog :title="dialog1.title" :visible.sync="dialog1.visible">
            <el-form  ref="dataForm" :model="tempData" label-position="left" label-width="70px" style='width: 90%; margin-left:5%;'>
                ...
                <el-form-item label="排序" prop="sortId">
                    <el-input-number v-model="tempData.sortId" :min="1" :max="10"></el-input-number>
                </el-form-item>        
                <el-form-item label="书籍" prop="icon">
                    <el-input v-model="tempData.tempBook" placeholder="输入书籍id"></el-input>
                    <el-button type="text" @click="tempData.books?tempData.books.push(tempData.tempBook):tempData.books=[tempData.tempBook]">添加</el-button>
                    
                    <el-tag @close="tempData.books.splice(tempData.books.indexOf(item))" closable type="success" v-for="item in tempData.books||[]" >{{item}}</el-tag>
                </el-form-item>                           
...
methods:{
    ...
    async createData(data){
        ...
        await request('/systemConfig/editConfig',{id:this.configs._id,block:JSON.stringify(this.block)})
        this.$message('创建成功!')
    ...
    async updateData(item){
        ...
        let data = await request('/systemConfig/editConfig',{id:this.configs._id,block:JSON.stringify(this.block)})
        this.$message(`编辑成功！`)
        this.dialog1.visible=false
        this.$forceUpdate();   
    },
