<template>
    <div>
        <el-card :body-style="{ padding: '15px' }">
            ...
            <el-table :data="banner" border>
                <el-table-column label="链接" prop="href">
                </el-table-column>
                <el-table-column label="图片">
                    <template scope="scope">
                        <img style="height:50px" :src="scope.row.img" >
                    </template>
                </el-table-column>
        ...
        <el-dialog :title="dialog1.title" :visible.sync="dialog1.visible">
            <el-form  ref="dataForm" :model="tempData" label-position="left" label-width="70px" style='width: 90%; margin-left:5%;'>
                <el-form-item label="链接" prop="label">
                ...
                <el-form-item label="缩略图" prop="img">
                    <el-upload class="avatar-uploader" action="/system/upload?type=images" :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                        <img v-if="tempData.img" :src="tempData.img" class="avatar" style="width:100%">
...
methods:{
    ...
    async createData(data){
        ...
        await request('/systemConfig/editConfig',{id:this.configs._id,banner:JSON.stringify(this.banner)})
        this.$message('创建成功!')
    ...
    async updateData(item){
        ...
        let data = await request('/systemConfig/editConfig',{id:this.configs._id,banner:JSON.stringify(this.banner)})
        this.$message(`编辑成功！`)
        this.dialog1.visible=false
        this.$forceUpdate();   
    },
